...
export function runAction(value) {
...
async function writeCode(script, code) {
  ...
  if (promoEl) {
    if (promoEl.contentEditable === 'true') {
      promoEl.textContent = '';
    }
    setValue(promoEl, '');
...
function getCartTotal(selectors, useInnerText = false, sendMetricOnce) {
  selectors = selectors.split(',');
  const candidateTotals = [];
  _.forEach(selectors, selector => {
    const $els = $(selector);
    _.forEach($els, $el => {
      const total = parsePrice(useInnerText ? $el.innerText : $el.innerHTML, sendMetricOnce);
...
export function isCouponPage(script) {
  return new Promise((res, rej) => {
    const sendMetricOnce = _.once(sendMetric);
    ...
    exponentialBackoff.on('ready', (number, delay) => {
      try {
        const cartTotal = getCartTotal(
          script.cartTotalSelector,
          Boolean(script.useInnerText),
          sendMetricOnce
        );
        const isCouponCustom = !!_.get(script, 'showCouponsSelector', '').trim().length
