      ...
      _.forEach(result.pageData.coupons, code => {
        if (!seenCouponCodes.has(code)) {
          sendMetric('track', 'codeApplied', {
            ...
            code,
            currentLocation: _.pick(window.location, ['hash', 'host', 'hostname', 'href', 'origin', 'pathname', 'port', 'search'])
          });
...
function getShopifySiteData(siteAPIData) {
  ...
  if (shopifyDomain) {
    ...
    siteAPIData.siteData.pageTypes.confirmationPage = {
      identifiers: {
        and: [
          {
            type: 'url',
