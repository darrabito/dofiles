...
class CouponHeader extends Component {
  onCloseClick() {
    ...
    this.props.onClosePopup('x', {ctaText: ctaButtonText});
  }
  ...
  render() {
    ...
    return (
      <header className="coupon-header">
        ...
        {runAutomatically && !running ? (
          <div className="group">
            {!autoCouponSettings ? (
              <h4 className="silver-link icon-x autocoup" onClick={this.onCloseClick.bind(this)} />
            ) : null}
            <Gear
              onClick={toggleAutomaticCouponSettings.bind(this)}
              style={{
                position: 'relative',
                margin: '0px 10px',
                top: 'auto',
