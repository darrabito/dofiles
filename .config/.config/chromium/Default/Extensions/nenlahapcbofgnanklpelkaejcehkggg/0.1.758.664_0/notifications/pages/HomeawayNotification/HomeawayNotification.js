...
class HomeawayNotification extends Component {
  ...
  render() {
    const {hideNotification: hide, listing, sourceListing: source} = this.state;
    ...
    const hideNotification = hide || _.isEmpty(result);

    return (
      <div
        className={
          hideNotification || _.isEmpty(result)
            ? 'disabled homeaway-notification-page'
            : 'homeaway-notification-page'
        }
        style={{
        ...
        <Motion
          ...
          {({opacity, y}) => (
            <div
                ...
                <section>
                  ...
                  <div className="button-wrapper">
                    <button
                      ...
                      View on {result.domain === 'HomeAway' ? 'HomeAway' : 'Vrbo'}
                    </button>
                  </div>
                </section>
              )}
              {result.exactMatch && !savings && !result.quotedRequested && allowUnquotedResults && (
                <section>
                  <h2>
                    Get <span className="green">{_.get(cashback, 'reward.amount') / 100}%</span> back in Wikibuy credits on the exact same
                    property.
                  </h2>
                  <div className="button-wrapper">
                    <button
              ...
              {!result.exactMatch && otherResults && !!otherResults.length && (
                <section>
                  <h2 style={{marginBottom: '10px'}}>Savings found nearby.</h2>
                  {_.map(
                    _.filter(results, r => !r.exactMatch),
                    (result, i) => {
  ...
  async onClickListing(url) {
    const clickEvent = 'viewHomeawayListing';
    const {clickId, redirectId} = generateAffiliateIds();

    sendMetric('trackClick', clickEvent, 'view listing', {
      domain: location.hostname.replace(/^www\./, ''),
      pagePath: location.pathname,
      clickId,
      redirectId
    ...
    if (hasFeature('use_redirect_v2')) {
      redirectUrl = generateAffiliateUrl({
        params: {
          clickEvent,
          destUrl: url,
          redirectId,
          subExperience: 'hotels_view_offer',
          vendorMetaId: _.get(this.props, 'view.listing.cashback.id')
        },
        isWbPublisher: true
      });
    } else {
      redirectUrl = `${WIKIBUY_API}/redirect?r=1&url=${encodeURIComponent(url)}&channel=cashback&clickId=${clickId}`;
    }
