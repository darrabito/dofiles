...
import OfferAvailableElsewhere from './components/OfferAvailableElsewhere';
...
const AvailableElsewhereResult = ({result}) => {
  const total = _.get(result, 'pricing.total');
  return (
    <div className="wikibuy-savings-component">
...
class ProductPageAnnotation extends Component {
  ...
  render() {
    ...
    const showAvailableElsewereNotif = !!(
    ...
    const savingsButtonClassName = `${
      complete
        ? `${savings || reward || hasOfferWhenUnavailable || showAvailableElsewereNotif ? 'savings' : 'no-savings'}`
        : 'searching'
    }`;
        ...
        <div
          ...
          onClick={() => {
            this.viewProductPage(
              showAvailableElsewereNotif ? 'availableElsewhereButton' : 'savingsButton',
              hasOfferWhenUnavailable || showAvailableElsewereNotif ? bestOfferWhenUnavailable.resultId : null
            );
          }}
          ...
          ) : showAvailableElsewereNotif ? (
            <AvailableElsewhereResult result={bestOfferWhenUnavailable} />
          ...
          {complete && (savings || reward) && showSavingsDetails ? (
            ...
            />
          ) : complete && showAvailableElsewereNotif && showSavingsDetails ? (
            <OfferAvailableElsewhere
              run={_.get(this.props, 'notification.run')}
              onCloseTooltip={() => this.hideSavingsDetails()}
              viewProductPage={this.viewProductPage.bind(this, 'continue')}
            />
