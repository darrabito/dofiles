...
class Offers extends Component {
  ...
  componentDidMount() {
    sendMetric('page', 'sitehubOffers', {
      ...
      vendorMetaId: _.get(this.props.view, 'site.siteData.cashback.id')
    });
  ...
  render() {
    ...
    if (type && hasFeature('auth_gate_coupons') && !this.state.suppressAccountGate) {
      return (
        <div className={`wb-Offers`}>
          <CreateAccountGate
            session={this.props.session}
            settings={this.props.settings}
            accountGateType={type}
            onSubmitAccountSuccess={() => {
              this.setState({suppressAccountGate: true});
            }}
          />
        </div>
      );
      ...
      <div className={`wb-Offers ${hasPersonalization ? '' : 'no-personalization'}`}>
        ...
        (reward && disableCoupons) ? (
          <CashBackBlock
            ...
            hasCoupons={!!_.get(coupons, 'length')}
          />
        ) : null}
        {(!!_.get(reward, 'categories') &&
          !(couponsThrottled && this.props.couponsType !== 'eeyore')) ||
        (!!_.get(reward, 'categories') && disableCoupons) ? (
          <CashBackCategories
            hasCoupons={coupons && coupons.length && !disableCoupons}
            reward={reward}
          />
        ) : null}
        {coupons && coupons.length && !disableCoupons ? (
          <CouponsBlock
            ...
            showAvailableCodes={currentDomain() !== 'ebay.com'}
          />
        ) : null}
