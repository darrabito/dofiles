...
import changeColors from 'utility/changeColors';

window.__wb_timing = window.__wb_timing || {
...
function setupWebComponent(options) {
  const template = $(`
    <template>
      <style>
        ${options.css ? options.css : `@import "${chrome.runtime.getURL(options.cssUrl)}";`}
...
export default async function loadApp(options, cb) {
  if (options.cssUrl) {
    const url = chrome.runtime.getURL(options.cssUrl);
    const res = await fetch(url);
    const css = await res.text();
    if (css) {
      options.css = await changeColors(css);
    }
  }
  ...
  // HIDE OTHER WB ELEMENTS  - Target was stacking. Dont want to break dom by removing.
  if (_.get(mountPoint, 'nextSibling.shadowRoot.querySelector') && mountPoint.nextSibling.shadowRoot.querySelector('.__wikibuy')) {
    mountPoint.nextSibling.style.display = 'none';
  }
