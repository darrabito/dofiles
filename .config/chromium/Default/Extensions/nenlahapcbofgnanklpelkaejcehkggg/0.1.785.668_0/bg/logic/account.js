...
import updateC1Ref from 'utility/updateC1Ref';
...
import {
  ...
  sendPinterestEvent,
  sendQuoraEvent
} from 'utility/pixelHelper';
import _ from 'lodash';
import getExtensionType from 'utility/getExtensionType';
import getFirefoxTrackingValue from 'utility/getFirefoxTrackingValue';
...
export async function refreshSession(options) {
  ...
  const session = options && options.session ? options.session : await getSession();
  ...
  if (Date.now() > prefCheck) {
    ...
    if (getExtensionType() === 'firefox') {
      const currentTrackingValue = await getFirefoxTrackingValue();
    ...
    if (sendEvents) {
      // Send "active user" events to ad channels
      sendTaboolaEvent('ActiveUser');
      sendOutbrainEvent('Active User');
      sendPinterestEvent('viewcategory');
      sendQuoraEvent('Search');
