...
export async function activateInPinnedTab({
  ...
  standDown(url, 'cashbackActivate', tabId);
  ...
  if (hasFeature('use_redirect_v2')) {
    ...
    const params = {
      destUrl: url,
      channel: 'cashback',
      subExperience: coupons ? 'coupons' : 'credits',
      redirectId: dashUuid(clickId),
      vendorMetaId,
      clickEvent: coupons
        ? fromAutoRun
          ? 'couponsModalClickCta'
          : 'tryCodesButton'
        : 'cashbackRedirect'
    };
    newAffUrl = affiliateUrl || generateAffiliateUrl({params});
  } else {
    newAffUrl =
      affiliateUrl ||
...
export default async ({url}) => {
  ...
  const siteAPIData = await getSite(hostname, url);
  ...
  if (cashback && cashback.reward && !isEbates && !disableCreditsTestGroup(domain, hasCoupons)) {
    ...
    cashback.hasCoupons = hasCoupons;
    return cashback;
  }
};
