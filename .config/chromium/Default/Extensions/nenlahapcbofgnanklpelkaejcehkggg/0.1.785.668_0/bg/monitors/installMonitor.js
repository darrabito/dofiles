...
function setSendInstallEventCookie() {
  chrome.cookies.set({
    url: WIKIBUY_URL,
    name: 'wb_needs_send_install_event',
    value: "true",
    expirationDate: moment()
      .add(1, 'year')
      .unix()
  ...
  setTimeout(async () => {
    analytics.track(
      ...
      {extensionVersion: version},
      {
        integrations: {
          'Customer.io': true,
          'Facebook Pixel': !hasFeature('prevent_fb_extensionInstalled')
        }
      }
    );
    ...
    if (sendEvents) {
      sendTaboolaEvent('extensionInstalled');
      sendOutbrainEvent('installActual');
