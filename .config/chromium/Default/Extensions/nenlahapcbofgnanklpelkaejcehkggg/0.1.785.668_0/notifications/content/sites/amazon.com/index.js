...
import getArrivalDate from '../../utility/getArrivalDate';
import checkParents from '../../utility/checkParents';
import {getPriceAsync} from '../../utility/getPrice';
import getShipping from '../../utility/getShipping';
...
function isLoaded() {
  // look for ATC or "see all buying options" button
  return !!(document.getElementById('addToCart') || document.getElementById('a-autoid-0-announce'));
}

function runASIN(asin, price, shipping, seller, atcData, product, feedback, dealId, arrivalDate) {
  if (!asin) {
    return;
  }
  ...
  startASIN({asin, price, shipping, seller, atcData, product, feedback, dealId, arrivalDate});
}
...
function getHiddenPrice(asin) {
  const baseProductPriceEl = document.querySelector('#attach-base-product-price');
  const baseProductPrice = baseProductPriceEl && baseProductPriceEl.getAttribute('value');

  const dataAsinPriceEl = document.querySelector(`[data-asin="${asin}"]`);
  const dataAsinPrice = dataAsinPriceEl && dataAsinPriceEl.getAttribute('data-asin-price');

  const ourPriceEl = document.querySelector('#priceblock_ourprice');
  const ourPrice = ourPriceEl && ourPriceEl.innerText;

  const bbScriptEl = document.querySelector('#new-toggle-buybox script[type="text/javascript"]');;
  const bbScriptPrice = bbScriptEl && regex(new RegExp(`a:"${asin}",bp:(\\\d+\\\.\\\d{0,2}),`), bbScriptEl.innerText);

  const price = baseProductPrice || dataAsinPrice || ourPrice || bbScriptPrice;
  if (price) {
    return parsePrice(parseFloat(price.replace('$', '')).toFixed(2));
  }
}
    ...
    if (!ignoreClicks || !getDetails) {
      ...
      if (getDetails) {
        ...
        if (parsers) {
          ...
          if (data) {
            price = data.price || getHiddenPrice(asin);
            product = getProductData(asin, data.title);
            shipping = data.shipping || getShipping(price);
            seller = data.seller || getSeller();
          }
        } else {
          price = await getPriceAsync(5) || getHiddenPrice(asin);
          shipping = getShipping(price);
          seller = getSeller();
      ...
      runASIN(asin, price, shipping, seller, atcData, product, feedback, dealId, arrivalDate);
    }
  } else {
    ignoreClicks = false;
  }
...
function checkGlobablIgnoreList(asin) {
  ...
  if (atcEl) {
    ...
    // we still want to run if there are new offers available, even if there is no ATC on page
    const newOffersLink = document.querySelector(`[href="/gp/offer-listing/${asin}/ref=dp_olp_new?ie=UTF8&condition=new"]`);
    if (
      text &&
      text.match(
...
async function asinUpdateCheck() {
  ...
  function onChange() {
    ...
    if (newAsin && newAsin !== currentASIN) {
      resetNotification();
      if (checkGlobablIgnoreList(newAsin) || checkIgnoreList(newAsinEl)) {
        ignoreRun();
  ...
  _.forEach(document.querySelectorAll('#activeCartViewForm [data-asin]'), itemEl => {
    ...
    if (priceEl) {
      loadEstimateAnnotation(
        {
          additionalClass: 'cart-page wbext-open-right',
