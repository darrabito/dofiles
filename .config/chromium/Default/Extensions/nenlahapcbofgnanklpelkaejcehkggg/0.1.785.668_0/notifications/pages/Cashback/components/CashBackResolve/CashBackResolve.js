...
import FtuxExplainer from './components/FtuxExplainer';
...
class CashBackResolve extends Component {
  ...
  onActivateLater(e) {
    ...
    const showFTUX =
      ...
      !_.get(this.props, 'hasSeenFTUX', false) &&
      !_.get(this.props, 'hasCoupons');

    if (showFTUX) {
      this.props.setSeenCashbackFtux();
      sendMetric('trackClick', 'dimissCashbackFtux', 'Continue Shopping', {
        domain: location.hostname.replace(/^www\./, ''),
        pagePath: location.pathname,
    ...
    return (
      <div className={`cash-back-resolve ${showIgnoreTooltip ? 'reminder-visible' : ''}`}>
        ...
        <div className="credit-prompt">
          {showFTUX ? (
            <FtuxExplainer
              exclusions={_.get(this.props, 'view.exclusions')}
              onActivate={this.props.onActivate}
              onActivateLater={this.onActivateLater.bind(this)}
              onShowExclusion={this.onShowExclusion.bind(this)}
              reward={_.get(this.props, 'reward')}
            />
