...
class Deals extends Component {
            ...
            <div
              ...
              {dealType === 'storageUnit' ? (
                ...
                <section>
                  <h2>{title}</h2>
                  <h4>
                    save {currency(discount, true)}
                    {_.get(this.props, 'view.queryData.title')
                      ? ` at ${_.get(this.props, 'view.queryData.title')}`
                      : ''}
                  </h4>
                  <div className="button-wrapper">
  ...
  async onClickDeal() {
    const clickEvent = 'viewGrouponDeal';
    const {clickId, redirectId} = generateAffiliateIds();

    sendMetric('trackClick', clickEvent, 'view deal', {
      domain: location.hostname.replace(/^www\./, ''),
      pagePath: location.pathname,
      clickId,
      redirectId
    ...
    const redirect = generateAffiliateUrl({
      params: {
        channel: 'cart',
        clickEvent,
        destUrl: `${_.get(this.props, 'view.deal.url')}&wid=${location.hostname}`,
        redirectId,
        subExperience: 'groupon_view_offer'
      },
      isWbPublisher: true
    });
