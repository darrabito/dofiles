...
import generateAffiliateIds from 'common/utility/generateAffiliateIds';
...
class Hotel extends Component {
  ...
  componentDidMount() {
    const wikibuyHotel = _.get(this.props, 'view.hotel.wikibuyHotel');

    if (wikibuyHotel) {
      const eventData = this.getWikibuyHotelEventData(wikibuyHotel);
      sendMetric('page', 'wbHotelNotification', eventData);
    } else {
  ...
  render() {
    ...
    const rewardAmount = _.get(cashback, 'reward.amount');
    const rewardText =
      _.get(cashback, 'reward.type') === 'percentage' ||
      _.get(cashback, 'reward.type') === 'forced-percentage'
        ? `${rewardAmount / 100}% back`
        : `${currency(rewardAmount)} back`;
                    ...
                    <div>
                      {discount > 0 && (
                        <React.Fragment>
                          ...
                          {rewardAmount > 0 && (
                            <h4>
                              Plus get <span className="palmetto">{rewardText}</span> on your stay
                              at {title}.
                            </h4>
                          )}
                        </React.Fragment>
                      ...
                      {discount === 0 && (
                        <h2>
                          Get <span className="palmetto">{rewardText}</span> on your stay at {title}
                          .
                        </h2>
                      )}
                    </div>
  ...
  getTextProperties() {
    const notification = ReactDOM.findDOMNode(this);
    const container = notification.querySelector('section');
    const button = notification.querySelector('button');
    const ctaText = button.innerText;

    return {
      notificationText: container.innerText.replace(`\n${ctaText}`, ''),
      ctaText
    };
  }
  ...
  async onClickDeal(url, preventHide) {
    ...
    if (wikibuyHotel) {
      sendMetric('trackClick', 'wbViewHotelDeal', 'view hotel', {
        ...this.getWikibuyHotelEventData(wikibuyHotel),
        dest_domain: 'wikibuy.com',
        dest_url: url
      });
      visitHotel({redirect: url});
    } else {
      const {clickId, redirectId} = generateAffiliateIds();
      const source = _.get(this.props, 'view.hotel.source');
      const useRedirectV2 = hasFeature('use_redirect_v2');
      ...
      sendMetric('trackClick', 'viewHotelDeal', 'view hotel', {
        ...
        pagePath: location.pathname,
        source,
        redirectId: useRedirectV2 ? redirectId : undefined,
        clickId,
        credits: !!_.get(this.props.view, 'hotel.cashback.reward.amount'),
        creditsAmount: _.get(this.props.view, 'hotel.cashback.reward.amount'),
        creditsType: _.get(this.props.view, 'hotel.cashback.reward.type')
      ...
      if (useRedirectV2) {
        redirect = generateAffiliateUrl({
          params: {
            clickEvent: 'viewHotelDeal',
            destUrl: redirect,
            redirectId,
            subExperience: 'hotels_view_offer'
          },
          isWbPublisher: true
        });
      } else if (_.get(this.props, 'view.hotel.cashback.reward.amount')) {
        redirect = `${WIKIBUY_API}/redirect?r=1&url=${encodeURIComponent(
          redirect
  ...
  onClosePopup(label) {
    ...
    if (wikibuyHotel) {
      const eventData = this.getWikibuyHotelEventData(wikibuyHotel);
      sendMetric('trackClick', 'wbDismissHotelNotification', label, eventData);
    } else {
  ...
  getWikibuyHotelEventData(wikibuyHotel) {
    const {address, id, name, offers} = wikibuyHotel;
    const searchParams = _.get(this.props, 'view.hotel.searchParams');

    return {
      ...this.getTextProperties(),
      ...searchParams,
      hotelId: id,
      hotelLocation: `${address.cityName}, ${address.stateCode}`,
      hotelName: name,
      pagePath: location.pathname,
      pricePostDiscount: offers[0].pricing.pricePostDiscount,
      pricePreDiscount: offers[0].pricing.pricePreDiscount,
      type: 'notification',
      url: location.href,
      view: 'hotelNotification'
    };
