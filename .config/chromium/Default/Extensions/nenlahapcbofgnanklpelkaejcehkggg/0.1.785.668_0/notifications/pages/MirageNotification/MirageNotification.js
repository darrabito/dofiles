...
const CLICK_EVENT = 'activateMirageNotification';
...
class MirageNotification extends Component {
  ...
  async onClickCTA() {
    ...
    const {vendorMetaId} = this.props.view;
    _.map(_.get(this.props.view, 'actions'), action => {
      if (action.type === 'openUrl' && _.get(action, 'props.href')) {
        const redirect = this.getRedirectUrl(action, redirectId, vendorMetaId);
        window.open(redirect);
      } else if (action.type === 'track') {
        sendMetric(
          'trackClick',
          action.event || CLICK_EVENT,
          '',
          _.assign(
            {
              domain: location.hostname.replace(/^www\./, ''),
      ...
      } else if (action.type === 'pinnedTab') {
        const redirect = this.getRedirectUrl(action, redirectId, vendorMetaId);
        this.pinnedTab(clickId, redirect);
      }
    });
  ...
  throttleNotif() {
    ...
    couponsMessenger({domain, message: 'throttle'});
  }

  async pinnedTab(clickId, redirect) {
    try {
  ...
  getRedirectUrl(action, redirectId, vendorMetaId) {
    const channel = _.get(action, 'props.channel', 'cashback');
    return generateAffiliateUrl({
      params: {
        channel,
        clickEvent: CLICK_EVENT,
        credits: channel === 'cashback' || channel === 'cashback-ims',
        destUrl: _.get(action, 'props.href'),
        redirectId,
        subExperience: 'mirage_notification',
        vendorMetaId
      },
      isWbPublisher: true
    });
