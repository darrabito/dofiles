...
class ProductEstimateAnnotation extends Component {
    ...
    return (
      <div className={`product-estimate-annotation ${hasTooltipOpen ? 'tooltip-open' : ''}`}>
        <div
          ...
          onClick={complete ? this.viewProductPage.bind(this, 'savingsButton') : () => {}}
          ...
          {filteredResults.length && this.state.showSavingsDetails ? (
            <OffersListAnnotationTooltip
              wbid={_.get(this.state, 'product.wbid')}
              originVendor={this.props.originVendor}
  ...
  visitMerchant(result) {
    ...
    if (paymentsSupport) {
      ...
      const clickEvent = 'visitMerchant';
      const {clickId, redirectId} = generateAffiliateIds();
      const hasReward = !!_.get(result, 'pricing.reward.amount');
      const redirect = generateAffiliateUrl({
        params: {
          channel: hasReward ? 'cashback-ims' : 'cart',
          clickEvent,
          destUrl: url,
          redirectId,
          subExperience: 'product_view_offer',
          vendorMetaId: _.get(result, 'pricing.reward.id')
        },
        isWbPublisher: true
      });
