...
export default async function loadCashbackNotification(deweyResult, options) {
  ...
  let showFTUX = false;
  if (hasFeature('ftux_continue_notification') || hasFeature('ftux_activate_notification')) {
    showFTUX = true;
    if (hasFeature('ftux_continue_notification')) {
      showCashback = true;
    }
  } else if (pageType === 'productPage' && !showCashback && hasFeature('mini_cashback_tab')) {
